  ...
  export class TWQXRefDataApi {
    ...
    public static readonly InsertOrUpdateWQXMonLoc =
      (monlocIdx: number, orgId: string, monlocId: string, monlocName: string,
        ...
        `${environment.api.owUrl}/api/monloc/InsertOrUpdateWQXMonLoc?mONLOC_IDX=${monlocIdx}&oRG_ID=${orgId}&mONLOC_ID=${monlocId}&mONLOC_NAME=${monlocName}&mONLOC_TYPE=${monlocType}&mONLOC_DESC=${monlocDesc}&hUC_EIGHT=${hucHeight}&HUC_TWELVE=${hucTwelve}&tRIBAL_LAND_IND=${tribalLandInd}&tRIBAL_LAND_NAME=${tribalLandName}&lATITUDE_MSR=${latitudeMsr}&lONGITUDE_MSR=${longitudeMsr}&sOURCE_MAP_SCALE=${sourceMapScale}&hORIZ_ACCURACY=${horizAccuracy}&hORIZ_ACCURACY_UNIT=${horizAccuracyUnit}&hORIZ_COLL_METHOD=${horizCollMethod}&hORIZ_REF_DATUM=${horizRefDatum}&vERT_MEASURE=${vertMeasure}&vERT_MEASURE_UNIT=${vertMeasureUnit}&vERT_COLL_METHOD=${vertCollMethod}&vERT_REF_DATUM=${vertRefDatum}&cOUNTRY_CODE=${countryCode}&sTATE_CODE=${stateCode}&cOUNTY_CODE=${countyCode}&wELL_TYPE=${wellType}&aQUIFER_NAME=${aquiferName}&fORMATION_TYPE=${formationType}&wELLHOLE_DEPTH_MSR=${wellholeDepthMsr}&wELLHOLE_DEPTH_MSR_UNIT=${wellholeDepthMsrUnit}&wQX_SUBMIT_STATUS=${wqxSubmitStatus}&drainageArea=${drainageArea}&drainageAreaUnit=${drainageAreaUnit}&contributingDrainageArea=${contrDrainageArea}&contributingDrainageAreaUnit=${contrDrainageAreaUnit} 
        &horizCollMethodName=${horizCollMethodName}&horizCordRefSysDatum=${horizCordRefSysDatum}
        &aquiferTypeName=${aquiferTypeName}&nationalAquiferCode=${nationalAquiferCode}
        &localAquiferCode=${localAquiferCode}&localAquiferCodeCtx=${localAquiferCodeCtx}
        &localAquiferDesc=${localAquiferDesc}
        &constructionDate=${constructionDate}
        &wellDepthMeasure=${wellDepthMeasure}&wellDepthMeasureUnit=${wellDepthMeasureUnit}&wQXUpdateDate=${wqxUpdateDate}&aCT_IND=${actInd}&wQX_IND=${wqxInd}&cREATE_USER=${createUser}`
    ...
    public static readonly insertOrUpdateWqxActivity =
      (activityIdx: number, orgId: string, projectIdx: number, monlocIdx: number, activityId: string,
        ...
        measureValue: string, gearProcUnitSelected: string, habitatSelMethod: string, methodName: string,
    ...
    public static readonly insertOrUpdateTWqxResult =
      (resultIdx: number, activityIdx: number, resultDetectCondition: string,
        ...
        targetCount: string, proportionSampProcNumeric: number, resultSampPointType: string,
        resultSampPointPlaceInSeries: string, resultSampPointCommentText: string, recordIdentifierUserSupplied: string,
        subjectTaxonomicNameUserSupplied: string, subjectTaxonomicNameUserSuppliedRefText: string, groupSummaryCount: string,
        functionalFeedingGroupName: string, comparableAnalMethodIdentifier: string, comparableAnalMethodIdentifierCtx: string,
