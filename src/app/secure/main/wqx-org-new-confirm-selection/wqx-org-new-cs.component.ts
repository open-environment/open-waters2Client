...
export class WqxOrgNewCSComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.currentOrgId !== ''){
      ...
      const data = this.service.getWQXOrganizationById(this.currentOrgId)
    .subscribe(
      (_data) => {
        if (_data != null) {
          ...
          if (org == null) {
            ...
            .subscribe(
              x => {
                this.eo = x;
                if (this.eo != null) {
                  const successID = this.service.InsertOrUpdateTWQXOrganization(this.eo.orgID, this.eo.orgFormalName, null, null, '', null, '', null, null, null, null, false, null, this.user.name, '', '', '', '');
                }else {
